...
xx move some of this to intro and chop downXX
...
Extending this idea to the global level, we  used a variety of data sources to estimate stock status for fisheries listed in RAM. Our goal is to simulate a macro-level global assessment, where data are available at regional levels, but not for specific fisheries. As a proof of concept, we first fit models to abundance indices from each stock in RAM. We assess performance by three metrics: median percent error (MPE, a measure of bias), median absolute percent error (MAPE, a measure of accuracy), and classification accuracy. Classification accuracy is calculated as the proportion of times that a given dataset results in a stock falling into the correct FAO status classification (one of underfished, maximally sustainably fished, and overfished). There are two main reasons why an assessment model might produce poor results: model misspecification and poor data. Stock assessment models included in RAM are generally age structured models containing a variety of nuances specific to individual species and fisheries. Since no global database of length or age composition data exists, global assessment methods are constrained to surplus production models in the manner used by `sraplus`. Assessing stock status through `sraplus` using the catch history and an index of abundance from RAM gives us a sense for how much model miss-specification might limit the ability of global assessment relative to data quality. Overall the `sraplus` estimates using the RAM data are reasonably good (median absolute percent error `r percent(perf$mape[perf$data == "ram-data"])`, accuracy  = `r percent(perf$accuracy[perf$data == "ram-data"])`, Table.\@ref(tab:perf-tab), Fig.\@ref(fig:mpe-map)-\@ref(fig:acc-map)). While these results are not perfect, this exercise tells us that given sufficiently high quality data, a surplus production model such as `sraplus` is reasonably able to reproduce the global state of fisheries as understood from formally assessed fisheries. 
...
Having knowledge of the state of the population in the most recent year reduced RMSE of the model estimates in the most recent five years by on average 20%. In terms of data however, information on fishing mortality rates (F/F~MSY~) in the most recent year was almost as useful as data on B/B~MSY~ itself, and having access to a recent string of estimates of F/F~MSY~ estimates was actually more useful Fig.\@ref(fig:voi-plot). Interestingly, having access to an estimate of F/F~MSY~ had a clearer and more substantial effect than having an index of abundance (xx uep, since in some cases the index was flatish but the stock status was low, so a model fit just to the trend in the abundance gets the trend right but the absolute value wrong. having F/Fmsy data helps anchor that). This is because we varied both the length and quality of the index of abundance, and on average an even noisy estimate of recent F/F~MSY~ may be more valuable than a short and noisy abundance index. 
...
`sraplus` contains too many options to cover within this methods section. We encourage readers to explore the documentation available at the package website at www.github.com/danovando/sraplus. Below we describe the structure of the population model underpinning `sraplus`, the estimation model, and a brief example demonstrating the use of the model. 
...
Where $B_t$ is biomass at time *t*, *r* is the intrinsic growth rate, *m* is the scaling parameter that allows for the ratio of B~msy~/K to shift. When *m* is two Bmsy / K = 0.5 and the model acts as a Schaefer model, lower values of m shift the production function left, higher values right. *c* is catch, and *p* is process error. Growth rates can become unrealistically large when the population reaches low sizes under the Pella-Tomlinson model. We deal with this problem by following the methods described in @winker2018. to reduce the production of the population when it falls below a threshold of 25% of carrying capacity. 
...
`sraplus` can be run in two forms: either as a stock reduction analysis [SRA,@walters2006], or fit to an index of abundance (fishery dependent or independent). Unless there is an abundance index to fit to the model runs as a stock reduction analysis. A stock reduction analysis works by specifying prior distributions on population parameters and critically the recent state of the fishery. `sraplus` allows users to specify the most recent status in units of depletion, B/B~MSY~, F, or F/F~MSY~. We then sample from the prior distributions of the population model parameters and apply those to the production model, along with the catch history. Any run that results in the collapse of the population (catch greater than biomass in any time step) is immediately rejected. The remaining viable draws from the prior distributions are sampled in proportion to the supplied prior on recent stock status. It is critical to note that a stock reduction analysis is only a mechanism for revealing implicit priors on fishery status and parameters through a sample-importance-resampling algorithm: the model cannot learn anything new about stock status in the absence of additional data to fit to, beyond the prior on stock status that is fed to it. The usefullness of this process then is in filling in the gap of parameters of interest (e.g. maximum sustainable yield, MSY), and providing historic values of stock status. Any model results based on data sources listed in Table.\@ref(tab:dat-description) that do not contain "cpue" or "RAM data" is estimated through stock reduction analysis. 

When an index of abundance is available the model estimates the posterior probability distributions of the estimated and transformed parameters using Hamiltonian Monte Carlo implemented in stan [@standevelopmentteam2018] accessed through the `tmbstan` interface [@monnahan2018]. The model fits to a direct estimate of abundance (e.g. a fishery independent survey or a standardized catch-per-unit-effort index), the likelihood is

$$log(\pmb{A_t}) \sim normal(f(r,k,m,B0,\pmb{p},\pmb{C}) \times q, sigma_{obs{}}) $$
where *f* is the Pella-Tomlinson produciton model (Equation 1.XX). When an effort index is available, `sraplus` constructs an index of abundance based on the catch and effort data. @rousseau2019 measure an index of abundance as catch divided by their effort index, either nominal or effective (assuming the 2.6\% technology rate). This assumes that every unit increase in effort is linearly greater than the unit of effort before it. When effort increases dramatically above historic levels, this can create a CPUE index that decreases faster than the true population. This is due to the fact that the marginal fishing mortality produced by increasing unit of efforts increases decreases as effort approaches infinity (since fishing mortality is bounded between 0 and 1). To accommodate this, we generate a catch per effective harvest rate index of abundance, as

$$cpue_t = \frac{catch_t}{(1 - e^{-q_tE_t})}$$
...
Priors for all estimable parameters can be left at default values or set by the user. The shape parameter is usually not reliably estimable given available data for surplus production models, however, @thorson2012 provides estimate the ratio of B~MSY~ to K for many fish taxa. While we estimate *m* by default throughout the results presented here, we use highly informative priors for the shape parameter drawn from @thorson2012 based on the genus of the species in question. 
...
## Example Use of `sraplus`

`sraplus` can be used in two general ways: as a mechanism for revealing implicit priors on fishery status through a sample-importance-resampling algorithm, and as a means for fitting indices of abundance informed by priors. To illustrate we will walk through a series of ways that `sraplus` could be applied to a case study fishery. 


What happens when we impose informative priors on the population parameters? For example at a bare minimum, we know that carrying capacity must be less than infinity, and we can certainly make inferences based on knowledge from other better studied fisheries of similar species on what a potential upper bound on carrying capacity might be. Once we have generated informative priors on our population parameters, we have implicitly provided priors on the biomass and therefore status of the fishery over its entire history: if our priors on carrying capacity only extend a small fraction beyond the minimum value needed to ensure the existence of the population, our implicit prior is that the population is severely depleted. If our prior is that carrying capacity is orders of magnitude greater than the observed catches, then our implicit prior is that the population is relatively unexploited. 

For our case study fishery, we can use `sraplus` as a filter to say, conditional on our priors, the catch history, and the population model, what is our implied prior on stock status of the fishery over time (see Section XX and for descriptions of model parameters). While user defined priors on population parameters can be supplied, `sraplus` has a number of built in mechanisms for setting parameters based on the taxonomy of the species in question and the magnitude of the catch history. This form of analysis is often extended though to include an explicit prior on stock status in the most recent year [@walters2006]. 

For our case study, expert opinion tells us that the stock is likely between XX and XX% of carrying capacity in the most recent year. While we can use `sraplus` to include this addition information, doing so introduces a nontrivial statistical problem: we have now placed two priors on the same outcome (current stock status), one explicit and one implicit in our growth parameter priors.  The net effect of this is that once these priors are run through `sraplus`, our post-model-pre-data "estimate" of stock status will appear to have been updated, creating an illusion that we have learned something about stock status. However, we have absolutely no data with which to update our prior on stock status beyond the requirement that the population exists. 

`sraplus` implements an approximate solution to this problem. Rather than using combinations of implied and explicit priors, `sraplus` asks, given an explicit prior on recent stock status, what prior distributions of population parameters produce our explicit prior on recent stock status, given the other prior distributions, catch history and model assumptions. This helps resolve some of the statistical issues related to placing multiple priors on the same parameter (recent stock status), and makes clear where any information on recent stock status is coming from. To return to our case study then, uncorrected, combining our explicit priors on recent stock status with our population parameter priors reveals an implicit prior on recent stock status that spans XX to XX. Using our correction, the implicit prior on recent stock status more or less matches our explicit prior (Fig.XX). 


While `sraplus` lets users supply their best estimate of recent stock status for a given fishery, the model also allows users to supply data on the swept area ratio (SAR , @amoroso2018) and/or fisheries management index (FMI, @melnychuk2017) scores for a given fishery. SAR is a measure of the trawling intensity in a given region, and as such for species susceptible to trawl gears can be an effective index of fishing pressure. FMI reflects expert opinion on the research, management, enforcement, and socioeconomic capacity of a given fishery, which together have significant associations with stock status. `sraplus` uses relationships between these SAR and FMI scores and observed F/F~MSY~  and B/B~MSY~ values to generate a posterior predictive distribution of F/F~MSY~ or B/B~MSY~ for the fishery in question, which serves as the priors for `sraplus`. This process lets us convert information on stock status present in other available data into a form that aligns with the model structure of `sraplus`.

To continue our case study, suppose that we no longer believe we have good data on current stock status, but do have a reasonable estimate of swept area ratio. `sraplus` can now use the posterior predictive $\hat{F/F~MSY~}$ generated by the SAR value as the prior for recent F/F~MSY~ for the fishery in question. This process allows users to translate something that they can measure, SAR, into something that they perhaps cannot (F/F~MSY~). The model then determines the growth parameter priors that conditional on the catch history and model structure produce the SAR generated $\hat{F/F~MSY~}$ prior distribution. 

Up to this point we still have not fit the model to any data; sraplus is simply helping us visualize the implications of our priors in terms of stock status over time and relevant reference points such as MSY. In order to actually learn something new about the stock in question, we need to update our priors with data. `sraplus` allows for two methods for accomplishing this: the model can be fit to an observed index of abundance (for example a fishery independent survey), or fit to a catch-per-unit effort index calculated from the observed catch and an index of fishing effort. In the latter case, the model must make an assumption about the rate of efficiency increase of a unit of fishing effort over time. When data are available the model can be provided priors on recent fishery status derived from expert opinion or FMI/SAR values as well. 
