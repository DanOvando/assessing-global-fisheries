...
XX discussion with Jim. How can we frame this. I would actually push back the idea that we have learned about stock status. I buy that the catch data can update your prior on growth parameters, but by the rules we're playing with here, it shouldn't update our prior on stock status should it. Now, the only thing I can't wrap my head aroudn is the case where catches are all zero, and you have a prior that the stock is collapsed. I guess my question is from a strikely Bayes theorem perspective, where in here is your prior on stock status being updated, since there's no likelihood as written, except for the binomial process that says that the stock exists. 

For our case study, we selected `r n_distinct(exs$stockid)` stocks for which we have stock specific Fisheries Management Index and swept area ratio scores. We then paired effort data at the resolution of year, country, and FAO statistical area from @rousseau2019 to each stock. We used these data to demonstrate how `sraplus` can add incremental amounts of information to an assessment, and the potential value of doing so. We first used the catch history heuristics internal to CMSY [@froese2017] to estimate stock status. We then used stock-specific data on swept area ratio and fisheries management index to generate priors on F/F~MSY~ for each of the stocks, which were then passed to `sraplus`. It is critical to note that in both of these cases the model has learned nothing new about recent stock status that was not encoded in the supplied priors, catch data, and functional form of the model; `sraplus` simply helps reveal our implicit priors on other parameters (e.g. maximum sustainable yield, MSY), and provides timeseries of measures such as B/B~MSY~. That, in the absence of data to actually fit to such as an index of abundance, methods such as CMSY or sraplus can learn anything new about a stock's status that is not already implicitly coded in the priors, catch data, or specified functional relationships, is a common misconception (see [Methods](#Methods) a for detailed explanation of why this is). Lastly, we used the reconstructed effort data from @rousseau2019 to create an index of abundance for each stock (see [Estimation Model](#Estimation Model) section), and estimated stock status by fitting to this index while using priors on fishing mortality rates informed by each stock's fisheries management index and swept area ratio values. 

Nearly all of the fisheries used in this case study have F/F~MSY~ values less than one, and most have B/B~MSY~ values greater than one. CMSY, using no stock-specific information but the catch history and the estimated resilience of the target species, badly misses this trend, stating instead that nearly all the fisheries in this group are currently overfished and experiencing overfishing. Adding priors informed by stock-specific the swept-area-ratio and fisheries management index data produces a dramatically different picture, capturing the general state that most of these fisheries have low fishing mortality rates and biomass near or above B~MSY~. Fitting to an index of abundance created by regional effort data in fact produces a slightly poorer fit. This is largely because a number of stocks in this case study set have very low catches (relative to historic highs) and high biomass values. An abundance index based on catch and regional effort cannot capture this trend, since it interprets the extremely low catches (near zero) as low biomass since we do not have sufficiently fine-scale effort data to reflect the reduction in effort that goes hand-in-hand with regulated catch reductions (Fig.\@ref(fig:cs-plot)). 
...
As our test case we use `r ram_comp_data$stockid %>% n_distinct()` fisheries from RAM with estimates of B/B~MSY~ and greater than 25 years of continuous catch history. B/B~MSY~ values from RAM are themselves estimates, not data, but they are the best available information on global stock status. We then pair the catch histories for these stocks with regional-level swept area ratio, fisheries management index, and effort data. For any given stock we use the area-weighted swept area ratio (where area refers to the spatial area of the SAR study in question) for the FAO region of the stock. For fisheries management index we use the mean component FMI values for the species group and FAO region in question (or FAO region if no matches exist at the species group level). This process approximates a global-level assessment exercise, where data are available at regional levels, but not for specific fisheries. We then constructed an effort index for the stock in question as the total effort from @rousseau2019 estimated in all countries that report catch of the species in question within its FAO region, weighting the effort of each country by its contribution to the total catch of that species in that FAO region. 
...
Having knowledge of the state of the population in the most recent year reduced RMSE of the model estimates in the most recent five years by on average 15%. Information on fishing mortality rates (F/F~MSY~) in the most recent year was equally useful as data on B/B~MSY~ itself, and having access to a recent string of estimates of F/F~MSY~ estimates was actually more useful Fig.\@ref(fig:voi-plot). Interestingly, having access to an estimate of F/F~MSY~ had a clearer and more substantial effect than having an index of abundance. While having access to a complete timeseries of an abundance index had a higher probability of reducing RMSE than increasing it, in many cases having only an abundance index resulted in a correct fit to the overall trend in abundance, but an error-prone estimate of B/B~MSY~ itself due to incorrectly estimating either the shape parameter, the initial state of the stock, or both. However, using an index of abundance that only covered the most recent quarter of a fishery's history actually increased RMSE by over 10% on average. 
...
`sraplus` can be run in two forms: either as a stock reduction analysis [SRA,@walters2006], or fit to an index of abundance (fishery dependent or independent). Unless there is an abundance index to fit to the model runs as a stock reduction analysis. A stock reduction analysis works by specifying prior distributions on population parameters and critically the recent state of the fishery. `sraplus` allows users to specify the most recent status in units of depletion, B/B~MSY~, F, or F/F~MSY~. We then sample from the prior distributions of the population model parameters and apply those to the production model, along with the catch history. Any run that results in the collapse of the population (catch greater than biomass in any time step) is immediately rejected. The remaining viable draws from the prior distributions are sampled in proportion to the supplied prior on recent stock status. 

It is critical to note that a stock reduction analysis is only a mechanism for revealing implicit priors on fishery status and parameters through a sample-importance-resampling algorithm: the model cannot learn anything new about stock status in the absence of additional data to fit to, beyond the prior on stock status that is fed to it and the constraints imposed by structural assumptions. The usefulness of this process then is in filling in the gap of parameters of interest (e.g. maximum sustainable yield), and providing historic values of stock status. Any model results based on data sources listed in Table.\@ref(tab:dat-desc) that do not contain "cpue" or "RAM data" is estimated through stock reduction analysis. All SRA style runs in our paper sampled 2,000 draws of the prior-predictive distribution from a total of 1e6 candidate draws. 
...
```{r}
plot_prior_posterior(co_fit, co_driors)
```
