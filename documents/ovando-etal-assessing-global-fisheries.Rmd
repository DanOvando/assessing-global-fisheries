...
plot_sraplus(`biased fit` = biased_co_fit, `unbiased fit` = co_fit)
...
pull out example from SAR fits
```{r}
sar <- sraplus::sar %>% 
  filter(stockid %in% unique(ram_data$stockid)) %>% 
  arrange(desc(sar)) %>% 
  select(stockid, everything()) %>% 
  slice(10)

ex <- ram_data %>% 
  filter(stockid %in% unique(sar$stockid)) 
...
sar_driors <- sraplus::format_driors(taxa = unique(ex$scientificname),
                                 catch = ex$catch, 
                                 years = ex$year,
                                 sar = sar$sar[sar$stockid == unique(ex$stockid)])

plot_driors(sar_driors)


sar_fit <- fit_sraplus(sar_driors, estimate_shape = TRUE)

plot_sraplus(`biased fit` = biased_co_fit, `unbiased fit` = co_fit, `sar fit` = sar_fit)


# fit using biomass index

index_driors <- sar_driors <- sraplus::format_driors(taxa = unique(ex$scientificname),
                                 catch = ex$catch, 
                                 years = ex$year,
                                 sar = sar$sar[sar$stockid == unique(ex$stockid)],
                                 index = ex$b_v_bmsy[!is.na(ex$b_v_bmsy)], 
                                 index_years = ex$year[!is.na(ex$b_v_bmsy)])

index_fit <- fit_sraplus(index_driors, engine = "stan", estimate_shape = TRUE)

plot_sraplus(`biased fit` = biased_co_fit, `unbiased fit` = co_fit, `sar fit` = sar_fit,
             `index fit` = index_fit,
             plotvars = "b_div_bmsy") + 
  geom_point(data = ex, aes(year, b_v_bmsy))
