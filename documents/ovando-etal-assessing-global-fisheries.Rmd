...
For our case study, expert opinion tells us that the stock is likely between XX and XX% of carrying capacity in the most recent year. While we can use `sraplus` to include this addition information, doing so introduces a nontrivial statistical problem: we have now placed two priors on the same outcome (current stock status), one explicit and one implicit in our growth parameter priors.  The net effect of this is that once these priors are run through `sraplus`, our post-model-pre-data "estimate" of stock status will appear to have "updated", creating an illusion that we have learned something about stock status. Remember though that we have absolutely no data on which to update our prior on stock status beyond the minimum floor needed to ensure that the population exists. 

`sraplus` implements an approximate solution to this problem. Rather than using combinations of implied and explicit priors, `sraplus` asks, given an explicit prior on recent stock status, what prior distributions of growth parameters produce our explicit prior on recent stock status, given the catch history and model assumptions. This helps resolve some of the statistical issues related to placing multiple priors on the same parameter, and makes clear where any information on recent stock status is coming from. To return to our case study then, uncorrected, combining our explicit priors on recent stock status with our population parameter priors reveals an implicit prior on recent stock status that spans XX to XX. Using our correction, the implicit prior on recent stock status more or less matches our explicit prior (Fig.XX). 

While `sraplus` lets users supply their best estimate of recent stock status for a given fishery, the model also allows users to supply data on the swept area ratio (SAR , @amoroso2018) and/or fisheries management index (FMI, @melnychuk2017) scores for a given fishery. SAR is a measure of the trawling intensity in a given region, and as such for species susceptible to trawl gears can be an effective index of fishing pressure. FMI reflects expert opinion on the research, management, enforcement, and socioeconomic capacity of a given fishery, which together have significant associations with stock status. `sraplus` uses relationships between these SAR and FMI scores and observed F/F~MSY~  and B/B~MSY~ values to generate a posterior predictive distribution of F/F~MSY~ or B/B~MSY~ for the fishery in question, which serves as the priors for `sraplus`. 

To continue our case study example, suppose that we no longer believe we have good data on current stock status, but do have a reasonable estimate of swept area ratio. `sraplus` now uses the posterior predictive F/F~MSY~ generated by the SAR value as the prior for recent F/F~MSY~ for the fishery in question. This process allows users to translate something that they can measure, SAR, into something that they perhaps cannot (F/F~MSY~). The model then determines the growth parameter priors that conditional on the catch history and model structure produce the SAR generated F/F~MSY~ prior
