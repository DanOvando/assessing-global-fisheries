...
cd <- data_frame(Parameter = "Carrying Capacity", Abbreviation = "$K$", `Default Prior` = "Prior predictive tuning") %>%
  ...
  rbind(c("Initial State", "$B/K_0$", "Posterior probability dist. of catch-based regressions"))
...
```{r si-prior-tab}
cd <- data_frame(Parameter = "Carrying Capacity", Abbreviation = "K", `Default Prior` = "$LN(10\\times{max(catch)},4)$") %>%
  rbind(c("Growth rate", "r", "$LN(r_{fishlife}, \\sigma_{r,fishlife})$")) %>%
  rbind(c("Shape parameter", "m", "Thorson et al. (2012)")) %>%
  rbind(c("Catchability", "q", "$LN(1e^{-3}, 0.3)$")) %>%
  rbind(c("Observation Error", "$\\sigma_{obs}$", "$LN(.05,2)$")) %>%
  rbind(c("Process Error", "$\\sigma_{proc}$", "$LN(.05,0.5)$")) %>% 
  rbind(c("Tech Creep", "$creep$", "$LN(0.025,0.2)$"))
